...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] = {
    b.legalNeighbors.map((bl: Block, mv: Move) => (bl, mv :: history)).to(LazyList)
  }
  ...
  def newNeighborsOnly(neighbors: LazyList[(Block, List[Move])],
    ...
    neighbors.filterNot((bl: Block, mv: List[Move]) => explored.contains(bl))
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    case (bl, mv) #:: tail =>
      val neighbours = newNeighborsOnly(neighborsWithHistory(bl, mv), explored)
      (bl, mv) #:: from(tail ++ neighbours, explored + bl)
  }
  ...
  lazy val pathsToGoal: LazyList[(Block, List[Move])] = {
    pathsFromStart.filter((bl: Block, mv: List[Move]) => done(bl))
  }
