...
trait EncoderInstances:
  ...
  given stringEncoder: Encoder[String] =
    Encoder.fromFunction(Json.Str.apply) // TODO Implement the `Encoder[String]` given instance
...
trait DecoderInstances:
  ...
  given listDecoder[A](using decoder: Decoder[A]): Decoder[List[A]] =
    ...
    def decodeAndPrepend(item: Json, previouslyDecodedItems: List[A]): Option[List[A]] =
      decoder.decode(item) match
        case Some(i) => Option(i +: previouslyDecodedItems)
        case _ => Option.empty
    ...
    Decoder.fromFunction {
      case Json.Arr(n) => decodeAllItems(n)
    }
  ...
  def field[A](name: String)(using decoder: Decoder[A]): Decoder[A] =
    Decoder.fromPartialFunction { case Json.Obj(n) => decoder.decode(n(name)).get }
...
trait PersonCodecs:
  ...
  given personEncoder: Encoder[Person] =
  ...
  given Decoder[Person] = Decoder.field[String]("name")
    ...
    .transform[Person](props => Person.apply(props._1, props._2))
...
trait ContactsCodecs:
  ...
  given Decoder[Contacts] = Decoder
    .field[List[Person]]("people")
    .transform[Contacts](prop => Contacts.apply(prop))
