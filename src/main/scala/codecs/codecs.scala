...
trait EncoderInstances {
  ...
  implicit val stringEncoder: Encoder[String] = Encoder.fromFunction(x => Json.Str(x)) // TODO Implement the `Encoder[String]` instance

  /** An encoder for `Boolean` values */
  // TODO Define an implicit value of type `Encoder[Boolean]`
  implicit val boolEncoder: Encoder[Boolean] = Encoder.fromFunction(x => Json.Bool(x))
...
trait DecoderInstances {
  ...
  implicit val stringDecoder: Decoder[String] = Decoder.fromPartialFunction { case Json.Str(x) => x }

  /** A decoder for `Boolean` values */
  // TODO Define an implicit value of type `Decoder[Boolean]`
  implicit val boolDecoder: Decoder[Boolean] = Decoder.fromPartialFunction { case Json.Bool(x) => x }
  ...
  implicit def listDecoder[A](implicit decoder: Decoder[A]): Decoder[List[A]] =
    Decoder.fromPartialFunction { case Json.Arr(x) => x.map(i => decoder.decode(i).get)}
  ...
  def field[A](name: String)(implicit decoder: Decoder[A]): Decoder[A] =
      Decoder.fromPartialFunction { case Json.Obj(x) => decoder.decode(x(name)).get }
}
...
trait PersonCodecs {
  ...
  implicit val personDecoder: Decoder[Person] = Decoder.field[String]("name").
    zip(Decoder.field[Int]("age")).transform[Person](info => Person(info._1, info._2))
}
